1.  Submission of documentation about what your data type would be used for, and how it fits into KBase. **You** *must* **wait for approval of this documentation before proceeding to 2.** This documentation should include at a minimum:
    ...
    2.  A document that describes in detail all elements of the diagram, what each entity is, why it is important, what it describes/contains and why and what the relationships mean and why they are important. You should include known applications (use cases) that use this data type, how it is used and why.

2.  A KBase typespec document that describe the implementation of your data type. This should be well documented to describe the implementation details of your type and how they relate to the conceptual model described by your conceptual diagram and documentation.
...
/src - source code that needs compiled, java or c/c++ or other compiled language
...
> KBase type names are according to the KBase Workspace module name and type name linked above under Workspace Typed Object Definition and Registration. An example would be KBaseGenomes.Genome. In a config file, we use that exact string representation, and in a file name for a script, “.” is replaced by “\_”.
>
> External type names are context specific and should be clearly specified and based on the external file format available. So if your data is an FBA model, and the format is Excel, then the external type name is Excel.FBAModel. So the template is \<external format name\>.\<context\>, where context could further be broken down, such as DNA.Reads or DNA.Assembly, etc. The context should be clear and distinguishable from other data.
...
> “max\_runtime”: \<how\_long\_in\_seconds\_before\_we\_should\_terminate\>,
>
> “must\_own\_validation”: \<true/false, if missing false; this means your transformation script is responsible for validation if true\>,
>
> “must\_own\_saving\_to\_workspace”: \<true/false, if missing false; this means your transformation script is responsible for saving objects to the workspace and filling out proper provenance information on save\>,
>
> “custom\_options”: \<list of custom field options, talk to us if you need this\>,
>
> “required\_fields”: \<list of field names you need\>,
...
Each script should minimally accept a working path where files can be created if needed. Below a list of input keywords are given that will be recognized if used in a config file. Anything outside of this list is not recognized and must be defined using a custom option in the config. If you need this behavior you should contact us to make sure we are able to support what you are trying to do.
...
>
> **working\_directory** - where you can write to, if needed
>
...
>
> **input\_mapping** - an input mapping that maps input keys
>
...
>
> Use this document to reference solr schema and config information, [*http://archive.apache.org/dist/lucene/solr/ref-guide/apache-solr-ref-guide-4.10.pdf*](http://archive.apache.org/dist/lucene/solr/ref-guide/apache-solr-ref-guide-4.10.pdf).
>
...
1.  Define a python script for pulling your workspace data and generating a tab delimited file that can be indexed by solr. Place your script under search/src/dataExtraction/solrLoad/, and you can find examples there as well.
...
3.  Sanitize the tab delimited file for characters that are problematic for ingest by solr. (I think we have a script for this but I don’t see it in the repo, need to find it)

4.  Ingest your tab delimited file into solr to generate the index using the load script in the search repo (install/bin/loadSolrCore.py) or the jar load tool provided by the solr distribution. (need more details here, things like getting solr up, etc)

5.  Define a search service plugin config that augments the default solr query and maps the solr output to fields returned by the service. You can see examples under (finish this). We do not directly expose solr internals because they are subject to change more frequently, i.e., extra fields may be defined for sorting or other purposes that don’t directly map back to the user. (need details on installing/running the service)
